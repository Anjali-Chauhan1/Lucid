...
Run: 300 synthetic samples (offline generator), 225 train / 75 held-out,
8 features, 4 classes.

| Model | 5-fold CV accuracy | Held-out accuracy |
|---|---|---|
| Logistic Regression *(exported)* | 0.720 ± 0.039 | 0.733 |
| Gradient Boosting | 0.720 ± 0.023 | **0.827** |

Per-class, held-out (Logistic Regression):

| Class | Precision | Recall | F1 | Support |
|---|---|---|---|---|
| good | 0.59 | 0.74 | 0.65 | 23 |
| partial | 0.58 | 0.50 | 0.54 | 22 |
| **memorized** | **1.00** | **1.00** | **1.00** | 15 |
| **wrong** | **1.00** | 0.80 | 0.89 | 15 |

![Confusion matrix](training/confusion_matrix.png)

**Reading these numbers honestly.** The two classes the product actually makes
claims about are near-perfect: **`memorized` is 1.00/1.00** and **`wrong` is
1.00 precision**. Almost all remaining error is the `good` ↔ `partial` boundary —
which is a coverage *continuum*, not a natural category boundary, and the
offline template generator makes those two deliberately similar (they differ
only by how many concept nodes were dropped). Overall accuracy is therefore
pessimistic relative to what the user experiences, because the Grasp Score
itself is a continuous number, not this label.

Gradient boosting outperforms LR on the held-out split (0.827 vs 0.733), but LR
is what gets exported: its coefficients port to TypeScript as a plain
`softmax(standardize(x) · coef + intercept)` with no tree-walking runtime, and
they are inspectable (see the coefficient table in `metrics_report.txt` — e.g.
`trigram_overlap` carries **+1.84** toward `memorized`, exactly as designed).
<!-- METRICS:END -->
...
|---|---|---|---|
| 5-fold CV accuracy | 0.720 | 0.613 | **+0.107** |
| Held-out accuracy | 0.733 | 0.693 | +0.040 |
| **`memorized` recall** | **1.00** | **0.80** | **+0.20** |
...
The n-gram overlap signal is what makes that distinction possible — and dropping
it costs a fifth of all memorization detection.
